...
export default function Success() {
  return (
    <section className="cv-auto py-32 bg-black px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {successMetrics.map((metric, index) => (
            <div
              key={metric.id}
              className={`bg-zinc-950 border border-zinc-900 rounded-3xl p-8 ${metric.colSpan} animate-fade-in-up`}
              style={{ animationDelay: `${index * 50}ms` }}
            >
              <div className="w-10 h-10 rounded-full border border-zinc-800 flex items-center justify-center text-zinc-500 text-sm font-mono mb-6">
                {metric.id}
              </div>
              <h3 className="text-xl font-semibold text-white mb-3 tracking-wide">{metric.title}</h3>
              <p className="text-zinc-400 leading-relaxed">{metric.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-4 bg-white text-black rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
          <div>
          ...
          <div className="shrink-0 w-12 h-12 rounded-full bg-black flex items-center justify-center text-white">
